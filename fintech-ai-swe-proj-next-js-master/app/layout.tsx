...
import { LayoutDashboard, TrendingUp, Bot, BarChart2, Globe, Search, Settings, Bell } from 'lucide-react';
        ...
        <div style={{ display: "flex", height: "100vh", position: "relative", zIndex: 1, overflow: "hidden" }}>
          ...
          {isMobile && (
            <div style={{
              ...
              zIndex: 100, overflowX: "auto",
            }}>
              {MOBILE_NAV.map(({ id, label, icon: Icon, href }) => {
                const act = pathname === href || (pathname === '/' && id === 'dashboard');
                return (
                  <Link key={id} href={href} style={{ textDecoration: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, padding: '6px 0', flex: 1, position: 'relative', minWidth: 60 }}>
                    <Icon size={18} color={act ? U.cyan : U.textMute} />
                    {id === 'notifications' && <span style={{ position: 'absolute', top: 4, right: '50%', marginRight: -10, width: 6, height: 6, borderRadius: '50%', background: U.rose }} />}
